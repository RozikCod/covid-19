      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          {files.map((file) => (
            <div
              key={file.id}
              className={`bg-white rounded-lg shadow-lg p-6 cursor-pointer transition-all duration-300 hover:shadow-xl hover:scale-105 ${selectedFile === file.id ? 'ring-2 ring-blue-500' : ''}`}
              onClick={() => setSelectedFile(selectedFile === file.id ? null : file.id)}
            >
              <div className="flex items-center mb-4">
                <div className={`p-3 rounded-lg ${file.color} text-white mr-4`}>
                  <file.icon size={24} />
                </div>
                <div>
        ...
        <div className="text-center">
          <p className="text-gray-600 mb-4">
            Click on any file card to view its contents and download it.
          </p>
          <div className="flex justify-center space-x-4">
            ...
            <div className="bg-white rounded-lg shadow p-4">
              <Layout className="mx-auto mb-2 text-purple-500" size={24} />
              <p className="text-sm font-semibold">Modern UI</p>
            </div>
          </div>
        </div>
      </div>
